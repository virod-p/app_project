...
class RWTFloorPlanPage extends StatefulWidget {
  final int floorNumber;
  const RWTFloorPlanPage({super.key, required this.floorNumber});

  @override
  _RWTFloorPlanPageState createState() => _RWTFloorPlanPageState();
}

class _RWTFloorPlanPageState extends State<RWTFloorPlanPage> {
  ...
  Future<void> fetchRooms() async {
    await Supabase.instance.client
        .from('rwt_floor${widget.floorNumber}')
        .select()
  ...
  Future<void> fetchRoomDetails(String roomName) async {
    await Supabase.instance.client
        .from(
            'rwt_floor${widget.floorNumber}') // เปลี่ยนเป็นชื่อ table ที่เก็บข้อมูลห้อง
        .select()
        .eq('name', roomName)
        .single()
        .then((response) {
